...
class BaseReporter:
    ...
    def run(
        ...
        if self.use_new_db_schema:
            ret = self.run_new(tab=tab, add_summary=add_summary, pagesize=pagesize, page=page, make_filtered_table=make_filtered_table, user=user)
        else:
            ret = self.run_old(tab=tab, add_summary=add_summary, pagesize=pagesize, page=page, make_filtered_table=make_filtered_table, user=user)
        return ret
    ...
    def write_data_new(
        ...
        self.cf.get_level_data_iterator(
            level, page=page, pagesize=pagesize, uid=self.ftable_uid, var_added_cols=self.var_added_cols
        )
        ctime = time.time()
    ...
    def write_data(
        ...
        cursor_read = conn_read.cursor()
        self.cf.get_level_data_iterator(
            level, page=page, pagesize=pagesize, uid=self.ftable_uid, var_added_cols=self.var_added_cols, cursor=cursor_read
        )
        ctime = time.time()
        if isinstance(cursor_read, sqlite3.Cursor):
            self.retrieved_col_names[level] = [d[0] for d in cursor_read.description]
    ...
    def add_gene_level_displayname_to_variant_level_columngroups(self, module_name, coldefs):
        conn = self.get_db_conn()
        cursor = conn.cursor()
    ...
    def add_gene_level_columns_to_variant_level(self):
        ...
        for module_name in modules_to_add:
            ...
            if not gene_coldefs:
                continue
            self.add_gene_level_displayname_to_variant_level_columngroups(module_name, gene_coldefs)
