...
from .cmd_run import parser as oakvar_cmd_parser
from .cmd_test import parser as oakvar_test_parser
from .cmd_web import parser as oakvar_web_parser
from .cmd_report import parser as oakvar_report_parser
import sys
...
# run
p_run = sp_entry.add_parser("run", parents=[oakvar_cmd_parser], add_help=False, description="Run a job", help="Run a job", epilog="inputs should be the first argument")

# report
p_report = sp_entry.add_parser("report", parents=[oakvar_report_parser], add_help=False, help="Generate a report from a job", epilog="dbpath must be the first argument")

# gui
p_gui = sp_entry.add_parser("gui", parents=[oakvar_web_parser], add_help=False, help="Start the GUI")
...
p_module_ls = sp_module.add_parser("ls", parents=[cmd_admin.parser_ls], add_help=False, help="List modules")
p_module_install = sp_module.add_parser("install", description="Install modules", parents=[cmd_admin.parser_install], add_help=False, help="Install modules")
...
p_module_installbase = sp_module.add_parser("installbase", parents=[cmd_admin.parser_install_base], add_help=False, help="Install base modules")
...
p_util_test = sp_util.add_parser("test", parents=[oakvar_test_parser], add_help=False, help="Test installed modules")
